...
function DetailsCards({ loadHelpPanelContent }) {
  ...
  const {
    ...
    filtering: {
      empty: <TableEmptyState resourceName="Tiles" />,
      noMatch: (
  ...
  useEffect(() => {
    new DataProvider().getData("tiles").then((distributions) => {
  ...
  return (
    <Cards
      ...
      filter={
        <TextFilter
          {...filterProps}
          filteringAriaLabel="Filter tiles"
          filteringPlaceholder="Find tiles"
...
function App() {
  ...
  return (
    <CustomAppLayout
      ref={appLayout}
      navigation={<Navigation activeHref="#/tiles" />}
